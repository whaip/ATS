...
{
    m_timeTicker.reset(new QCPAxisTicker);
...
double UESTCQCustomPlot::visibleLowerBound() const
{
    return xAxis ? xAxis->range().lower : 0.0;
}

double UESTCQCustomPlot::visibleUpperBound() const
{
    return xAxis ? xAxis->range().upper : 0.0;
}

void UESTCQCustomPlot::resetAutoView()
{
    resumeAutoRange();
}

int UESTCQCustomPlot::plotAreaWidth() const
{
    return axisRect() ? axisRect()->rect().width() : width();
}
...
QCPGraph *UESTCQCustomPlot::addRealTimeLine(const QString &name, const QColor &color)
{
    ...
    const QPen basePen(useColor, 2);
    graph->setPen(basePen);
    graph->setSelectable(QCP::stNone);
    m_realTimeGraphs.insert(name, graph);
    m_graphBasePens.insert(graph, basePen);
                                           ...
                                           const QVector<QVector<double>> &buffers)
{
    ...
    xBuffers.reserve(buffers.size());
    for (const auto &buffer : buffers) {
        QVector<double> x;
        x.reserve(buffer.size());
        for (int i = 0; i < buffer.size(); ++i) {
            x.push_back(static_cast<double>(i) * static_cast<double>(m_sampleIntervalUs) / 1000000.0);
        }
        xBuffers.push_back(x);
    }
    updateRealTimeSeries(graphs, xBuffers, buffers, true);
}
                                           ...
                                           const QVector<qint64> &intervalsUs)
{
    ...
    for (int i = 0; i < buffers.size(); ++i) {
        QVector<double> x;
        x.reserve(buffers[i].size());
        const double intervalSec = static_cast<double>(qMax<qint64>(1, intervalsUs[i])) / 1000000.0;
        for (int j = 0; j < buffers[i].size(); ++j) {
            x.push_back(static_cast<double>(j) * intervalSec);
        }
        xBuffers.push_back(x);
    }
    updateRealTimeSeries(graphs, xBuffers, buffers, true);
...
void UESTCQCustomPlot::updateRealTimeSeries(const QVector<QCPGraph *> &graphs,
                                            const QVector<QVector<double>> &xBuffers,
                                            const QVector<QVector<double>> &yBuffers,
                                            bool autoFollow)
{
    if (graphs.isEmpty() || graphs.size() != xBuffers.size() || graphs.size() != yBuffers.size()) {
        return;
    }
    ...
    for (int i = 0; i < graphs.size(); ++i) {
        ...
        const auto &x = xBuffers[i];
        const auto &y = yBuffers[i];
    ...
    if (m_autoRangeEnabled && !m_userOverride && hasRange) {
        yAxis->setRange(minVal, maxVal);
        applyYAxisPadding();
    }

    if (autoFollow && !m_userOverride && hasLatest) {
        xAxis->setRange(0.0, qMax(m_displayWindowSeconds, latestX));
    }

    replot(QCustomPlot::rpQueuedReplot);
}

bool UESTCQCustomPlot::computeVisibleYRange(double xLower, double xUpper, double *outMin, double *outMax) const
{
    if (!outMin || !outMax) {
        return false;
    }
    ...
    for (int i = 0; i < graphCount(); ++i) {
        ...
        auto begin = data->findBegin(xLower, true);
        auto end = data->findEnd(xUpper, true);
...
void UESTCQCustomPlot::applyVisibleAutoRange(bool resetX)
{
    if (!m_autoRangeEnabled) {
        return;
    }

    const double xLower = resetX ? 0.0 : visibleLowerBound();
    const double xUpper = resetX ? m_displayWindowSeconds : visibleUpperBound();
    double minVal = 0.0;
    double maxVal = 0.0;
    if (computeVisibleYRange(xLower, xUpper, &minVal, &maxVal)) {
        yAxis->setRange(minVal, maxVal);
        applyYAxisPadding();
    }
...
void UESTCQCustomPlot::updateStaticLine(QCPGraph *graph, const QVector<double> &x, const QVector<double> &y, bool rescale)
{
...
void UESTCQCustomPlot::handleLegendClick(QCPLegend *legendPtr, QCPAbstractLegendItem *item, QMouseEvent *event)
{
    ...
    refreshLegendItemAppearance(graph);
    if (!m_userOverride) {
        applyVisibleAutoRange(false);
    }
    deselectAll();
    replot(QCustomPlot::rpQueuedReplot);
}
...
void UESTCQCustomPlot::refreshLegendItemAppearance(QCPGraph *graph)
{
    ...
    const QPen basePen = m_graphBasePens.value(graph, graph->pen());
    if (graph->visible()) {
        graph->setPen(basePen);
    } else {
        QColor muted = basePen.color();
        muted.setAlpha(80);
        graph->setPen(QPen(muted, basePen.widthF(), Qt::DashLine));
    }
}
