...
ClockTpsPlugin::ClockTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString ClockTpsPlugin::pluginId() const { return QStringLiteral("tps.clock"); }
QString ClockTpsPlugin::displayName() const { return QStringLiteral("Clock TPS"); }
QString ClockTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> ClockTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement ClockTpsPlugin::requirements() const
{
...
bool ClockTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("clock TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for clock.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool ClockTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool ClockTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("clock TPS framework executed");
