...
DisplayTpsPlugin::DisplayTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString DisplayTpsPlugin::pluginId() const { return QStringLiteral("tps.display"); }
QString DisplayTpsPlugin::displayName() const { return QStringLiteral("Display TPS"); }
QString DisplayTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> DisplayTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement DisplayTpsPlugin::requirements() const
{
...
bool DisplayTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("display TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for display.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool DisplayTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool DisplayTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("display TPS framework executed");
