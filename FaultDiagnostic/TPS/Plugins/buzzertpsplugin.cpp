...
BuzzerTpsPlugin::BuzzerTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString BuzzerTpsPlugin::pluginId() const { return QStringLiteral("tps.buzzer"); }
QString BuzzerTpsPlugin::displayName() const { return QStringLiteral("Buzzer TPS"); }
QString BuzzerTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> BuzzerTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement BuzzerTpsPlugin::requirements() const
{
...
bool BuzzerTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("buzzer TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for buzzer.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool BuzzerTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool BuzzerTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("buzzer TPS framework executed");
