...
ConnectorTpsPlugin::ConnectorTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString ConnectorTpsPlugin::pluginId() const { return QStringLiteral("tps.connector"); }
QString ConnectorTpsPlugin::displayName() const { return QStringLiteral("Connector TPS"); }
QString ConnectorTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> ConnectorTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement ConnectorTpsPlugin::requirements() const
{
...
bool ConnectorTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("connector TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for connector.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool ConnectorTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool ConnectorTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("connector TPS framework executed");
