...
BatteryTpsPlugin::BatteryTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString BatteryTpsPlugin::pluginId() const { return QStringLiteral("tps.battery"); }
QString BatteryTpsPlugin::displayName() const { return QStringLiteral("Battery TPS"); }
QString BatteryTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> BatteryTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement BatteryTpsPlugin::requirements() const
{
...
bool BatteryTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("battery TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for battery.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool BatteryTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool BatteryTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("battery TPS framework executed");
