#include "potentiometertpsplugin.h"
...
PotentiometerTpsPlugin::PotentiometerTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString PotentiometerTpsPlugin::pluginId() const { return QStringLiteral("tps.potentiometer"); }
QString PotentiometerTpsPlugin::displayName() const { return QStringLiteral("Potentiometer TPS"); }
QString PotentiometerTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> PotentiometerTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement PotentiometerTpsPlugin::requirements() const
{
...
bool PotentiometerTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("potentiometer TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for potentiometer.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool PotentiometerTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool PotentiometerTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("potentiometer TPS framework executed");
