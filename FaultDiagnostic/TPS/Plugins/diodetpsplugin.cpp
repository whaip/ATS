...
DiodeTpsPlugin::DiodeTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString DiodeTpsPlugin::pluginId() const { return QStringLiteral("tps.diode"); }
QString DiodeTpsPlugin::displayName() const { return QStringLiteral("Diode TPS"); }
QString DiodeTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> DiodeTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement DiodeTpsPlugin::requirements() const
{
...
bool DiodeTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("diode TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for diode.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool DiodeTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool DiodeTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("diode TPS framework executed");
