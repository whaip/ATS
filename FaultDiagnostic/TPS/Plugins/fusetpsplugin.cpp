...
FuseTpsPlugin::FuseTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString FuseTpsPlugin::pluginId() const { return QStringLiteral("tps.fuse"); }
QString FuseTpsPlugin::displayName() const { return QStringLiteral("Fuse TPS"); }
QString FuseTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> FuseTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement FuseTpsPlugin::requirements() const
{
...
bool FuseTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("fuse TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for fuse.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool FuseTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool FuseTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("fuse TPS framework executed");
