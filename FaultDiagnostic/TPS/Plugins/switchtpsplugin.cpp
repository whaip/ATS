...
SwitchTpsPlugin::SwitchTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString SwitchTpsPlugin::pluginId() const { return QStringLiteral("tps.switch"); }
QString SwitchTpsPlugin::displayName() const { return QStringLiteral("Switch TPS"); }
QString SwitchTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> SwitchTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement SwitchTpsPlugin::requirements() const
{
...
bool SwitchTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("switch TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for switch.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool SwitchTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error)
{
    Q_UNUSED(error)
    m_settings = settings;
    return true;
...
bool SwitchTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId;
    result->success = true;
    result->summary = QStringLiteral("switch TPS framework executed");
