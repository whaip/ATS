...
ICTpsPlugin::ICTpsPlugin(QObject *parent)
    : QObject(parent)
{
}

QString ICTpsPlugin::pluginId() const { return QStringLiteral("tps.ic"); }
QString ICTpsPlugin::displayName() const { return QStringLiteral("IC TPS"); }
QString ICTpsPlugin::version() const { return QStringLiteral("1.0.0"); }
QVector<TPSParamDefinition> ICTpsPlugin::parameterDefinitions() const { return requirements().parameters; }

TPSPluginRequirement ICTpsPlugin::requirements() const
{
...
bool ICTpsPlugin::buildDevicePlan(const QVector<TPSPortBinding> &bindings, const QMap<QString, QVariant> &settings, TPSDevicePlan *plan, QString *error)
{
    ...
    devicePlan.wiringSteps = {QStringLiteral("IC TPS framework created. Wiring strategy will be configured later.")};
    devicePlan.temperatureGuide = QStringLiteral("Select the temperature ROI near %1 for IC.").arg(anchorText(settings, QStringLiteral("temperatureAnchor")));
...
bool ICTpsPlugin::configure(const QMap<QString, QVariant> &settings, QString *error) { Q_UNUSED(error) m_settings = settings; return true; }

bool ICTpsPlugin::execute(const TPSRequest &request, TPSResult *result, QString *error)
{
    if (!result) { if (error) *error = QStringLiteral("result is null"); return false; }
    result->runId = request.runId; result->success = true; result->summary = QStringLiteral("IC TPS framework executed");
