...
LedDiagnosticPlugin::LedDiagnosticPlugin(QObject *parent)
    : QObject(parent)
{
}

QString LedDiagnosticPlugin::pluginId() const
{
    return QStringLiteral("diag.led");
}

QString LedDiagnosticPlugin::displayName() const
{
    return QStringLiteral("LED Diagnostic");
}

QString LedDiagnosticPlugin::version() const
{
    return QStringLiteral("1.0.0");
}

QString LedDiagnosticPlugin::componentType() const
{
    return QStringLiteral("tps.led");
}

DiagnosticReport LedDiagnosticPlugin::diagnose(const DiagnosticInput &input, QString *error) const
{
    ...
    QStringList lines;
    lines.push_back(QStringLiteral("LED diagnostic framework created."));
